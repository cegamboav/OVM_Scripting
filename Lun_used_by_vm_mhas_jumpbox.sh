...
collect_mulipath(){
	...
	java -jar /bin/tools/icmd/icmd-1.1.1.jar -e -cmd "sudo multipath -ll" -u $user -s tmp_dir_luns/temp_server_file.csv -t $Change -cwd /usr/bin/ -sync -fq -n demo-exec1 |grep -v INFO > tmp_dir_luns/$1.mltp.txt
	java -jar /bin/tools/icmd/icmd-1.1.1.jar -e -cmd "sudo multipathd show maps status" -u $user -s tmp_dir_luns/temp_server_file.csv -t $Change -cwd /usr/bin/ -sync -fq -n demo-exec1 |grep -v INFO > tmp_dir_luns/$1.status.txt
}

##This function is used to collect the vm.cfg file of the running vms:
collect_vms_conf_file(){
	create_server_file $1
	vmFile=$(java -jar /bin/tools/icmd/icmd-1.1.1.jar -e -cmd "sudo find /OVS/Repositories/ -name $2 |head -1" -u $user -s tmp_dir_luns/temp_server_file.csv -t $Change -cwd /usr/bin/ -sync -fq -n demo-exec1 |grep -v INFO)
	java -jar /bin/tools/icmd/icmd-1.1.1.jar -e -cmd "sudo cat $vmFile/vm.cfg" -u $user -s tmp_dir_luns/temp_server_file.csv -t $Change -cwd /usr/bin/ -sync -fq -n demo-exec1 |grep -v INFO > tmp_dir_luns/$2.vms.txt
}
...
collect_xm_list(){
	...
	create_server_file $1
	java -jar /bin/tools/icmd/icmd-1.1.1.jar -e -cmd "sudo xm list" -u $user -s tmp_dir_luns/temp_server_file.csv -t $Change -cwd /usr/bin/ -sync -fq -n demo-exec1 |grep -v INFO > tmp_dir_luns/$1.xmli.txt
}
